...
class POWERec(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.num_modal = 3
        self.emb_size = config['embedding_size']    # int type: the embedding size
        self.dropout = config['dropout']            # float32 type: the dropout rate of LayerGCN
        self.reg_weight = config['reg_weight']      # float32 type: the weight for l2 normalizaton
        self.neg_weight = config['neg_weight']      # float32 type: the weight for hard negatives
        self.prompt_num = config['prompt_num']      # int type: the number of prompt embeddings
        ...
        ## define prompt embeddings for modalities
        self.id_prompt = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.prompt_num, self.emb_size)))
        self.v_prompt = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.prompt_num, self.emb_size)))
        self.t_prompt = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.prompt_num, self.emb_size)))

        ## learn modality-specific user interest
        self.id_model = LayerGCN(self.n_users, self.n_items, self.user_embds, self.item_embds, self.emb_size, self.id_prompt)
        self.v_model = LayerGCN(self.n_users, self.n_items, self.user_embds, self.v_feat, self.emb_size, self.v_prompt)
        self.t_model = LayerGCN(self.n_users, self.n_items, self.user_embds, self.t_feat, self.emb_size, self.t_prompt)
    ...
    def bpr_loss(self, u_embeddings, i_embeddings, user, pos_item, neg_item):
        ...
        rank_loss = torch.sum(-m(pos_scores - neg_scores))

        weak_modality, modality_indicator = self.identify_weak_modality(u_embeddings, posi_embeddings, negi_embeddings)
        ...
        hard_rank_loss = torch.sum(-m(pos_scores - fake_neg_scores))
        return rank_loss, hard_rank_loss
    ...
    def identify_weak_modality(self, user_e, pos_e, neg_e):
    ...
    def calculate_loss(self, interaction):
        ...
        rank_loss, hard_rank_loss = self.bpr_loss(user_all_embeddings, item_all_embeddings, user, pos_item, neg_item)
        reg_loss = self.emb_loss(user, pos_item, neg_item)
        loss = rank_loss + self.neg_weight * hard_rank_loss + self.reg_weight * reg_loss
        return loss
